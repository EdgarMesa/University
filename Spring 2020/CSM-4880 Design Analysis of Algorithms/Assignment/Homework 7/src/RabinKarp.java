...
public class RabinKarp {
    ...
    public ArrayList<Integer> serachAll(String text)
    {
        ...
        int n = text.length(); 
        if (n < m) return occurences;
        long txtHash = hash(text, m); 

        // check for match at offset 0
        if ((patHash == txtHash) && check(text, 0))occurences.add(0);
    ...
    public static void main(String[] args) {
        ...
        int count = searcher.count(txt);
        occurences = searcher.serachAll(txt);
