...
X_train = pd.read_csv('..\\Decision_Tree\\train.csv')
X_test = pd.read_csv('..\\Decision_Tree\\test.csv')
y_test = pd.read_csv('..\\Decision_Tree\\gender_submission.csv')
...
# Cross validation to see which height is the best, will also get the best score and the bests height
for i in heights:
    ...
    score_g = cross_val_score(tree_gini, X, y, cv=10, scoring="accuracy")
    score_e = cross_val_score(tree_entropy, X, y, cv=10, scoring="accuracy")

    acc_gini = score_g.mean()
    scores_gini.append(acc_gini)
    acc_entropy = score_e.mean()
    scores_entro.append(acc_entropy)
...
plt.plot(heights, scores_gini, label="Gini")
plt.plot(heights, scores_entro, label="Entropy")
plt.xlabel("Max Number Tree Height")
plt.ylabel("Cross-Validated Accuracy")
plt.title("Gini and Entropy Tree")
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig("Cross_Validation_Trees")
plt.show()
